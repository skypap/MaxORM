...
//void checkPreus_vcodes(char c, int prev_code, char prev_code_len, int c_96, char l_96) {
//   if (prev_code != c_96 || prev_code_len != l_96) {
//     printf("Code mismatch: %d: %d!=%d, %d!=%d\n", c, prev_code, c_96, prev_code_len, l_96);
//   }
//}
...
void init_coder() {
  ...
  for (int i = 0; i < 7; i++) {
    for (int j = 0; j < 11; j++) {
      ...
      if (c != 0 && c != 32) {
        int ascii = c - 32;
        //int prev_code = c_96[ascii];
        //int prev_code_len = l_96[ascii];
        switch (i) {
          case SHX_SET1: // just us_vcode
            c_96[ascii] = (us_vcodes[j] << (16 - us_vcode_lens[j]));
            l_96[ascii] = us_vcode_lens[j];
            //checkPreus_vcodes(c, prev_code, prev_code_len, c_96[ascii], l_96[ascii]);
            if (c >= 'a' && c <= 'z') {
              ascii -= ('a' - 'A');
              //prev_code = c_96[ascii];
              //prev_code_len = l_96[ascii];
              c_96[ascii] = (2 << 12) + (us_vcodes[j] << (12 - us_vcode_lens[j]));
              l_96[ascii] = 4 + us_vcode_lens[j];
            }
            break;
          case SHX_SET1A: // 000 + us_vcode
            c_96[ascii] = 0 + (us_vcodes[j] << (13 - us_vcode_lens[j]));
            l_96[ascii] = 3 + us_vcode_lens[j];
            //checkPreus_vcodes(c, prev_code, prev_code_len, c_96[ascii], l_96[ascii]);
            if (c >= 'a' && c <= 'z') {
              ascii -= ('a' - 'A');
              //prev_code = c_96[ascii];
              //prev_code_len = l_96[ascii];
              c_96[ascii] = (2 << 12) + 0 + (us_vcodes[j] << (9 - us_vcode_lens[j]));
              l_96[ascii] = 4 + 3 + us_vcode_lens[j];
            }
            break;
          case SHX_SET1B: // 00110 + us_vcode
            c_96[ascii] = (6 << 11) + (us_vcodes[j] << (11 - us_vcode_lens[j]));
            l_96[ascii] = 5 + us_vcode_lens[j];
            //checkPreus_vcodes(c, prev_code, prev_code_len, c_96[ascii], l_96[ascii]);
            if (c >= 'a' && c <= 'z') {
              ascii -= ('a' - 'A');
              //prev_code = c_96[ascii];
              //prev_code_len = l_96[ascii];
              c_96[ascii] = (2 << 12) + (6 << 7) + (us_vcodes[j] << (7 - us_vcode_lens[j]));
              l_96[ascii] = 4 + 5 + us_vcode_lens[j];
            }
            break;
          case SHX_SET2: // 0011100 + us_vcode
            c_96[ascii] = (28 << 9) + (us_vcodes[j] << (9 - us_vcode_lens[j]));
            l_96[ascii] = 7 + us_vcode_lens[j];
            break;
          case SHX_SET3: // 0011101 + us_vcode
            c_96[ascii] = (29 << 9) + (us_vcodes[j] << (9 - us_vcode_lens[j]));
            l_96[ascii] = 7 + us_vcode_lens[j];
            break;
          case SHX_SET4: // 0011110 + us_vcode
            c_96[ascii] = (30 << 9) + (us_vcodes[j] << (9 - us_vcode_lens[j]));
            l_96[ascii] = 7 + us_vcode_lens[j];
            break;
          case SHX_SET4A: // 0011111 + us_vcode
            c_96[ascii] = (31 << 9) + (us_vcodes[j] << (9 - us_vcode_lens[j]));
            l_96[ascii] = 7 + us_vcode_lens[j];
        }
        //checkPreus_vcodes(c, prev_code, prev_code_len, c_96[ascii], l_96[ascii]);
      }
  ...
  for (l=0; l<len; l++) {
    ...
    if (c_in >= 32 && c_in <= 126) {
      ...
      if (c_in == 0 && state == SHX_STATE_2)
        ...
        ol = append_bits(out, ol, c_96[c_in], l_96[c_in], state);
    } else
